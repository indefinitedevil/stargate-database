...
@foreach ($results as $result)
    <li>
        ...
        @if (!empty($result['body_change']))
            ({{ __('Body +:change', ['change' => $result['body_change']]) }})
        @endif
        @if (!empty($result['vigor_change']))
            ({{ __('Vigor +:change', ['change' => $result['vigor_change']]) }})
        @endif
        @if (!empty($result['temp_body_change']))
            ({{ __('Body +:change for next event', ['change' => $result['temp_body_change']]) }})
        @endif
        @if (!empty($result['temp_vigor_change']))
            ({{ __('Vigor +:change for next event', ['change' => $result['temp_vigor_change']]) }})
        @endif
    </li>
@endforeach
</ul>
